% Create a function to find hL and K based on EAATS%
% Lower One-sided VSI 4 Regions Run Sum X-bar Chart for Gamma %
% Steady State %

function [hL, K] = EAATSFindhLKLowerVsiRSXbarGam(EAATS0_target, ASI0_target, n, hS, G, S1, S2, S3, S4, a_shape, b_scale)
    ...
    ub = [1.50, 2.0]; % Upper bounds for hL and K

    % Objective function with penalty for deviation from tau0
    obj_fun = @(x) ConstraintsEAATS(x, EAATS0_target, ASI0_target, n, hS, G, S1, S2, S3, S4, a_shape, b_scale) ; 
