% EAATS-Optimal Parameters for VSI Lower One-sided 4 Regions RS for Gamma %
% Steady State % 
...
for S1 = 0:1
    for S2 = S1:3
        for S3 = S2:5 
            for S4 = S3:10
                [hL, K] = EAATSFindhLKLowerVsiRSXbarGam(EAATS0_target, ASI0_target, n, hS, G, S1, S2, S3, S4, a_shape, b_scale); 
                EAATS0 = AATSFindGam(n, K, 0, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
                ASI0 = ASIFindGamSteady(n, K, 0, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ;
                if abs(EAATS0 - EAATS0_target) < 1 
                    EAATS = EAATSFindLowerGam(n, K, deltamin, deltamax, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
