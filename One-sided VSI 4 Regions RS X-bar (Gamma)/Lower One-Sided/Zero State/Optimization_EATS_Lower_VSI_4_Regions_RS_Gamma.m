% EATS-Optimal Parameters for VSI Lower One-sided 4 Regions RS X-bar Chart for Gamma %
% Zero State % 
...
for S1 = 0:1
    for S2 = S1:3
        for S3 = S2:5 
            for S4 = S3:10
                [hL, K] = EATSFindhLKLowerVsiRSXbarGam(EATS0_target, ASI0_target, n, hS, G, S1, S2, S3, S4, a_shape, b_scale); 
                ...
                if abs(EATS0 - EATS0_target) < 1 
                    EATS = EATSFindLowerGam(n, K, deltamin, deltamax, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
